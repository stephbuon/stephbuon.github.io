...
layout: post
title: 'PANGeA'
description: PANGeA is a method that uses large language models (LLMs) to create narrative content for turn-based RPGs based on game designers' high-level criteria. It introduces a novel validation system for handling free-form text during development and gameplay, employing "self-reflection" techniques. It enriches player-NPC interactions using the Big Five Personality model and custom memory system, and integrates seamlessly with game engines via a REST interface. PANGeA is highly effective in aligning LLM responses with game narratives, enhancing performance of local LLMs to match large-scale ones, like GPT-4, in fostering dynamic, narrative-consistent engagement.
